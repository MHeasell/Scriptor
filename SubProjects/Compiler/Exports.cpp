...
LPCTSTR BOSCOM_GetKWList(LPVOID pBosCom,int Index,int& Next)
{
	...
	if(Index==0) BosCom->Cmd->PrepareKWList();
...
LRESULT BOSCOM_Compile(LPVOID pBosCom,LPCTSTR BosPath,LPCTSTR CobPath,
					   SETTINGS Settings,
					   OUTPUTWINCALLBACK OutputWinCallback,
					   PROGRESSCALLBACK ProgressCallback)
{
	BOSCOM* BosCom=(BOSCOM*)pBosCom;
	CBosSrcParse BosSrc;
	BosSrc.ParseBos(BosPath,*(BosCom->Cmd),
					Settings,
					OutputWinCallback,
					ProgressCallback);
...
LRESULT BOSCOM_Destroy(LPVOID pBosCom)
{
	if(!pBosCom) return 1;
	BOSCOM* BosCom=(BOSCOM*)pBosCom;
	if(BosCom->Cmd) delete BosCom->Cmd;
	delete BosCom;
	return 1;
}