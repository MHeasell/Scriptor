...
int CCobValBuf::AddItem(CCobValBuf* ValBuf)
{
	int x;
	if(!ValBuf) return 0;
	if(HaveChild) return ChildBuf->AddItem(ValBuf);
	IncMax(ValBuf->GetLength());
	for(x=0;x<ValBuf->GetLength();x++) Buffer[Length+x]=ValBuf->Buffer[x];
...
int CCobValBuf::AddItem(CCobCmdBuf* CmdBuf)
{
	int x;
	if(!CmdBuf) return 0;
	if(HaveChild) return ChildBuf->AddItem(CmdBuf);
	IncMax(CmdBuf->GetLength());
	for(x=0;x<CmdBuf->GetLength();x++) Buffer[Length+x]=CmdBuf->Buffer[x];
...
int CCobValBuf::AddItem(WORD Type,long Val1,long Val2)
{
	...
	if(HaveChild)
	{
		...
		if(x==2)
		{
			...
			for(x=0;x<ChildBuf->GetLength();x++) Buffer[Length+x]=ChildBuf->Buffer[x];
...
int CCobCmdBuf::AddItem(char Flag,long item1,long item2)
{
	...
	else if( Flag & POST_CMD )
	{
		if( Flag & SPOT_BEGIN )
		{
			...
			if( x == 2 ) Buffer[CmdPos+2]=item2;
			Length+=x;
		}
		else
		{
			Buffer[Length]=item1;
			if( x == 2 ) Buffer[Length+1]=item2;
			Length+=x;
		}
	}
...
{
	if( !Cmd || !Cmd->CanDump() ) return 0;
	if( Cmd->GetLength()+CurOffset > MaxLength )
	{
		long* temp=ScriptBuf;
		MaxLength=MaxLength<<1;
		ScriptBuf=new long[MaxLength];
		for(int x=(Length-1);x>=0;x--) ScriptBuf[x]=temp[x];
		delete [] temp;
	}
