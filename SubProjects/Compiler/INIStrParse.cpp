...
void CINIStrParse::HandleVarVal(CString Var,CString Val)
{
	...
	if(InCommands)
	{
		if(Locale==1)
		{
			if(Var.CompareNoCase("Keyword")==0) Cmd.Command=Val;
	...
	{
		if(Var.CompareNoCase("Name")==0) Const.Name=Val;
		else if(Var.CompareNoCase("Value")==0) Const.Val=Val;
	}
	else if(InOps&&(Locale==1))
	{
		if(Var.CompareNoCase("Operator")==0) Op.Op=Val;
		else if(Var.CompareNoCase("Value")==0) Op.Val=strtol(Val,0,16);
		else if(Var.CompareNoCase("Priority")==0) Op.Priority=strtol(Val,0,10);
		else if(Var.CompareNoCase("LeftOnly")==0)
		{
			if(Val=="0") Op.LeftOnly=0;
			else Op.LeftOnly=1;
		}
	}
...
void CINIStrParse::HandleLocaleChange(int up)
{
	...
	else
	{
		...
		{
			if(ConLst) ConLst->Add(Const);
		}
		else if(InOps&&(Locale==1))
		{
			if(OpLst) OpLst->Add(Op);
		}
	}
}
...
WORD CINIStrParse::ParseStr(CString& Str,DWORD* Arg1,DWORD* Arg2)
{
	...
	switch(Flag)
	{
	case  1:HandleCategory(Val);
			WantVar=true;
			break;
	case  2:HandleLocaleChange(1);
			Locale++;
			break;
	case  3:HandleLocaleChange(0);
			Locale--;
			break;
	case  5:if(WantVar)
			{
