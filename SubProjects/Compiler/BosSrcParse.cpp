...
{
	if(ProgressCallback) ProgressCallback(CurPos,EndPos);
}
...
/*	else if( isdigit(*TxtBuf) || (*TxtBuf=='.') ||
	...
	else if( IsNum( *TxtBuf, *(TxtBuf+1), !(*LookingFor)) )
	{
		...
		*IdentTypeFlag=TYP_NUM;
		if(IsFloat) *ExFlag1=NTYP_FLOAT;
		else if(IsHex) *ExFlag1=NTYP_HEX;
		else if(IsOct) *ExFlag1=NTYP_OCT;
		else *ExFlag1=NTYP_DEC;
	...
	if(LastPos) *CharsFromLast = (TxtBuf-LastPos);
	...
	if(LastLine!=Line_Count) *PassedEOL=1;
	else *PassedEOL=0;
	// Save our current line to check next time
	if(!IgnoreLine) LastLine = Line_Count;
...
int CBosSrcParse::HandleRet(DATA* &TxtBuf,CString RetStr,WORD Ret)
{
	...
	switch(Ret)
	{
	case RET_INSERT:if(RetStr=="") break;
					...
					break;
	case RET_LINE_CMNT:for(;*TxtBuf!=0x0D;TxtBuf++)
						   if((TxtBuf-BufferData.TxtStart)>=BufferData.BufLength)
							   break;
						Line_Count++;
						TxtBuf++;
						break;
	case RET_BLCK_CMNT:for(;!((*TxtBuf=='*')&&(*(TxtBuf+1)=='/'));TxtBuf++)
						   if((TxtBuf-BufferData.TxtStart)>=BufferData.BufLength)
							   break;
							else if(*TxtBuf==0x0D) Line_Count++;
						TxtBuf+=2;
						break;
	case RET_NEXT_LINE:for(;*TxtBuf!=0x0D;TxtBuf++)
						   if((TxtBuf-BufferData.TxtStart)>=BufferData.BufLength)
							   break;
							   ...
							   PROGRESSCALLBACK Prog)
{
	...
	Bos=CreateFile(Path,
				   ...
				   NULL);
	if (Bos == INVALID_HANDLE_VALUE) return 0;
