...
int CCobCodec::PushOp(long Operator)
{
	...
	if (((Operator >= OP_ADD) && (Operator <= OP_RANDOM)) || ((Operator >= OP_LESS) && (Operator <= OP_OR)))
	{
		STACK.Pop(&Val);
...
void CCobCodec::Pop(StackVal Val, WORD Flags, int First)
{
	...
	StackVal newVal;
#define POPNEWVAL                                                                                                        \
	if (newVal.IsOpString() && (GetOpPriority(OpString[newVal.Val].Operator) < GetOpPriority(OpString[Index].Operator))) \
	{                                                                                                                    \
		NeedPara = TRUE;                                                                                                 \
	...
	if (Val.IsOpString())
	{
		...
		if (((Operator >= OP_ADD) && (Operator <= OP_BIT_OR)) || ((Operator >= OP_LESS) && (Operator <= OP_OR)))
		{
...
int CCobCodec::NextIsZero()
{
	StackVal Val;
	STACK.Peek(&Val);
	if ((Val.Flags() == TYP_CONST) && (Val.Val == 0))
		return 1;
	else
		return 0;
}
...
int CCobCodec::IsIncDec(long What, BYTE Flag)
{
	...
	StackVal Val, A, B;
	STACK.Peek(&Val);
	if (Val.IsOpString() && ((OpString[Val.Val].Operator == OP_ADD) || (OpString[Val.Val].Operator == OP_SUBTRACT)))
	{
		OpString[Val.Val].Values.Pop(&A);
		OpString[Val.Val].Values.Pop(&B);
		if ((A.Flags() & Flag) && (A.Val == What) && (B.Flags() & TYP_CONST) && (B.Val == 1))
...
void CCobCodec::PrintSndConsts(long Val)
{
	...
	{
		case 4:
			fout << "PLAYSOUND_PRIORITY_BATTLE";
			break;
		default:
			fout << std::dec << Val;
	}
	...
	if (TAK)
	{
		switch (Val)
		{
			...
			case 32:
				...
				break;
			case 256:
				fout << "EXPTYPE_SMALLEXPLOSION";
				break;
			case 512:
				fout << "EXPTYPE_MEDIUMEXPLOSION";
				break;
			case 1024:
				fout << "EXPTYPE_LARGEEXPLOSION";
				break;
			case 2048:
				fout << "EXPTYPE_XLARGEEXPLOSION";
				break;
			case 4096:
				fout << "EXPTYPE_BLOODEXPLOSION";
				break;
			case 8192:
				fout << "EXPTYPE_SMALLSMOKE";
				break;
			case 16384:
				fout << "EXPTYPE_MEDIUMSMOKE";
				break;
			case 65536:
				fout << "EXPTYPE_XLARGESMOKE";
				break;
			default:
				fout << std::dec << Val;
		}
	...
	while (count < Length)
	{
		...
		switch (*DataPtr)
		{
			...
			case 0x10065000:
				...
				{
					//STACK.Pop(&StackVal_1);
					if (ShowReturn || ((count + 1) < Length) || (tab > 1) || (!NextIsZero()))
					{
						TabSpace(tab, count);
						fout << "return (";
						//PrintOp(StackVal_1);
...
#define JUMP_OFFSET GET_END_OPCODE(-1)
				...
				if (ShowPush)
					fout << " // else JUMP -> " << std::hex << *(DataPtr + 1);
				fout << "\n";
				...
				if (WasLoop)
					Loops.RemoveAt(Loops.GetUpperBound());
...
long CCobCodec::ProcessScript(LPSTR Name, char* ScriptCode, long offset, long Length)
{
	if (StdLibs)
	{
		...
		if (TAK)
		{
			if (HaveMaxDFlames && HaveMaxSmokePuffs && (stricmp(Name, "SmokeControl") == 0))
			{
...
int CCobCodec::CheckAnimFunc(LPCTSTR Func)
{
	for (int x = AnimFunc.GetUpperBound(); x >= 0; x--)
		if (AnimFunc[x].CompareNoCase(Func) == 0)
			return TRUE;
	return FALSE;
}
...
LRESULT CCobCodec::OpenCob(LPCSTR FileName)
{
	...
	File = CreateFile(FileName, GENERIC_READ, (FILE_SHARE_READ | FILE_SHARE_WRITE), NULL, OPEN_EXISTING, (FILE_ATTRIBUTE_NORMAL | FILE_FLAG_RANDOM_ACCESS), NULL);
	...
	if (Header.VersionSignature == 6)
		TAK = true;
	else
		TAK = false;

	if (TAK)
		OffsetToFirstName = Header.OffsetToSoundNameArray + (Header.NumberOfSounds * 4);
	else
		OffsetToFirstName = Header.OffsetToNameArray;
...
LRESULT CCobCodec::MakeBos(LPCSTR FileName, CProgressCtrl* Prog)
{
	...
	if (TAK)
		Size += Header.NumberOfSounds; // Number of offsets to sound names
	...
	if (TAK)
		fout << "#define TAK			// This is a TAK script\n\n";
	else
		fout << "#define TA			// This is a TA script\n\n";

	fout << "#include \"sfxtype.h\"\n#include \"exptype.h\"\n\n";

	if (Header.NumberOfPieces)
		fout << "piece  ";
	...
	if (StaticVar.GetSize())
		fout << "static-var  " << std::dec;
	...
	if (Prog)
		Prog->SetRange(0, Header.NumberOfScripts);
	if (Prog)
		Prog->SetStep(1);
	...
	for (x = 0; x < Header.NumberOfScripts; x++)
	{
		...
		if (x < (Header.NumberOfScripts - 1))
			...
			Size = (Header.OffsetToScriptCode + (Header.LengthOfScripts * 4)) - Offset;
...
int CCobCodec::MakeSomeGuesses_TA(long Offset, long Length, long NumScripts)
{
	...
	if (Ret >= 0)
	{
		if (Ret + 1 == NumScripts)
			...
			End = OffsetArray[Ret + 1];
		for (Spot = OffsetArray[Ret]; Spot < End; Spot++)
		{
			if ((ScriptBuf[Spot] == CMD_STACK_ALC) && (ScriptBuf[Spot + 1] == CMD_STACK_ALC) && (ScriptBuf[Spot + 2] == CMD_STACK_ALC))
			{
	...
	if (Ret >= 0)
	{
		...
		for (Spot = OffsetArray[Ret]; Spot < End; Spot++)
		{
			if ((ScriptBuf[Spot] == CMD_POP_SVAR) && (ScriptBuf[Spot - 1] == OP_NOT) && (ScriptBuf[Spot - 3] == CMD_PUSH_SVAR) && (ScriptBuf[Spot - 2] == ScriptBuf[Spot + 1]))
			{
	...
	if (!StdLibs)
	{
		...
		if (Ret >= 0)
		{
			...
			for (Spot = OffsetArray[Ret]; Spot < End; Spot++)
			{
				if ((ScriptBuf[Spot] == CMD_POP_SVAR) && (ScriptBuf[Spot - 2] == CMD_PUSH_CON) && (ScriptBuf[Spot - 1] != 0))
				{
	...
	if (Ret >= 0)
	{
		...
		for (Spot = OffsetArray[Ret]; Spot < End; Spot++)
		{
			if ((ScriptBuf[Spot] == CMD_SIGNAL) && (ScriptBuf[Spot - 2] == CMD_PUSH_CON) && (ScriptBuf[Spot - 1] != 0))
			{
...
Exit:
	...
	if (Ret >= 0)
	{
		...
		for (Spot = OffsetArray[Ret]; Spot < End; Spot++)
		{
			if ((ScriptBuf[Spot] == CMD_CALL_SCRPT) && (ScriptBuf[Spot + 2] == 0))
			{
		...
		for (Spot = OffsetArray[Ret]; Spot < End; Spot++)
		{
			if ((ScriptBuf[Spot] == CMD_PUSH_SVAR) && (ScriptBuf[Spot + 2] == CMD_JUMP_IF) && (ScriptBuf[Spot + 4] == CMD_PUSH_FVAR) && (ScriptBuf[Spot + 5] == 0) && (ScriptBuf[Spot + 6] == CMD_POP_SVAR))
			{
...
int CCobCodec::MakeSomeGuesses_TAK(long Offset, long Length, long NumScripts)
{
	...
	if (Ret >= 0)
	{
		...
		for (Spot = OffsetArray[Ret]; Spot < End; Spot++)
		{
			if ((ScriptBuf[Spot] == CMD_POP_FVAR) && (ScriptBuf[Spot + 1] == 3) && (ScriptBuf[Spot - 1] == OP_RANDOM) && (ScriptBuf[Spot - 4] == 0))
			{
				if (!HaveMaxDFlames)
				{
					HaveMaxDFlames = true;
		...
		for (Spot = OffsetArray[Ret]; Spot < End; Spot++)
		{
			if ((ScriptBuf[Spot] == CMD_POP_FVAR) && (ScriptBuf[Spot + 1] == 2) && (ScriptBuf[Spot - 1] == OP_DIVIDE) && (ScriptBuf[Spot - 4] == 1000))
			{
	...
	if (Ret >= 0)
	{
		...
		for (Spot = OffsetArray[Ret]; Spot < End; Spot++)
		{
			if ((ScriptBuf[Spot] == CMD_POP_FVAR) && (ScriptBuf[Spot + 1] == 3) && (ScriptBuf[Spot - 1] == OP_MULTIPLY))
			{
				if (!HaveMaxDFlames)
				{
					HaveMaxDFlames = true;
	...
	if (Ret >= 0)
	{
		...
		for (Spot = OffsetArray[Ret]; Spot < End; Spot++)
		{
			if ((ScriptBuf[Spot] == CMD_POP_SVAR) && (ScriptBuf[Spot - 1] == OP_GET_UNITVAL) && (ScriptBuf[Spot - 3] == CMD_PUSH_CON) && (ScriptBuf[Spot - 2] == 32))
			{
	...
	if (Ret >= 0)
	{
		...
		for (Spot = OffsetArray[Ret]; Spot < End; Spot++)
		{
			if ((ScriptBuf[Spot] == CMD_POP_SVAR) && (ScriptBuf[Spot - 2] == CMD_PUSH_CON) && (ScriptBuf[Spot - 1] != 0) && (ScriptBuf[Spot - 5] == CMD_STRT_SCRPT) && (strnicmp(PrintScriptName(ScriptBuf[Spot - 4]), "WakeControl", MAX_PATH) == 0))
			{
	...
	if (Ret >= 0)
	{
		...
		for (Spot = OffsetArray[Ret]; Spot < End; Spot++)
		{
			if ((ScriptBuf[Spot] == CMD_POP_SVAR) && (ScriptBuf[Spot - 1] == OP_RANDOM))
			{
	...
	if (Ret >= 0)
	{
		...
		for (Spot = OffsetArray[Ret]; Spot < End; Spot++)
		{
			if ((ScriptBuf[Spot] == CMD_SLEEP) && (ScriptBuf[Spot - 2] == CMD_PUSH_SVAR))
			{
				StaticVar[ScriptBuf[Spot - 1]].Name = "restore_delay";
				StaticVar[ScriptBuf[Spot - 1]].Type = TYP_INTEGER;
				break;
	...
	if (Ret >= 0)
	{
		...
		for (Spot = OffsetArray[Ret]; Spot < End; Spot++)
		{
			if ((ScriptBuf[Spot] == CMD_POP_SVAR) && (ScriptBuf[Spot - 1] == OP_GET_UNITVAL) && (ScriptBuf[Spot - 3] == CMD_PUSH_CON) && (ScriptBuf[Spot - 2] == 29))
			{
		...
		for (Spot = OffsetArray[Ret]; Spot < End; Spot++)
		{
			if ((ScriptBuf[Spot] == CMD_POP_SVAR) && (ScriptBuf[Spot - 2] == CMD_PUSH_CON) && (ScriptBuf[Spot - 1] != 0) && (ScriptBuf[Spot - 5] == CMD_CALL_SCRPT) && (strnicmp(PrintScriptName(ScriptBuf[Spot - 4]), "restore", MAX_PATH) == 0))
			{
	...
	for (x = 0; x < NumScripts; x++)
		if ((strnicmp(PrintScriptName(x), "MeleeControl", MAX_PATH) == 0) || (strnicmp(PrintScriptName(x), "DemonControl", MAX_PATH) == 0) || (strnicmp(PrintScriptName(x), "RestoreWatcher", MAX_PATH) == 0))
			Ret = x;
	if (Ret >= 0)
	{
		if (Ret + 1 == NumScripts)
		...
		for (Spot = OffsetArray[Ret]; Spot < End; Spot++)
		{
			if ((ScriptBuf[Spot] == CMD_POP_SVAR) && (ScriptBuf[Spot - 2] == CMD_PUSH_CON) && (ScriptBuf[Spot - 1] != 0) && (ScriptBuf[Spot - 5] == CMD_CALL_SCRPT) && (strnicmp(PrintScriptName(ScriptBuf[Spot - 4]), "restore_legs", MAX_PATH) == 0))
			{
		...
		for (Spot = OffsetArray[Ret]; Spot < End; Spot++)
		{
			if ((ScriptBuf[Spot] == CMD_POP_SVAR) && (ScriptBuf[Spot - 2] == CMD_PUSH_CON) && (ScriptBuf[Spot - 1] != 0) && (ScriptBuf[Spot - 5] == CMD_CALL_SCRPT) && ((strnicmp(PrintScriptName(ScriptBuf[Spot - 4]), "restore_x", MAX_PATH) == 0) || (strnicmp(PrintScriptName(ScriptBuf[Spot - 4]), "restore", MAX_PATH) == 0)))
			{
				StaticVar[ScriptBuf[Spot + 1]].Name = "restored";
				StaticVar[ScriptBuf[Spot + 1]].Type = TYP_BOOL;
				break;
	...
	if (Ret >= 0)
	{
		...
		for (Spot = OffsetArray[Ret]; Spot < End; Spot++)
		{
			if ((ScriptBuf[Spot] == CMD_POP_SVAR) && (ScriptBuf[Spot - 2] == CMD_PUSH_CON) && (ScriptBuf[Spot - 4] == CMD_JUMP_IF))
			{
	...
	if (Ret >= 0)
	{
		...
		for (Spot = OffsetArray[Ret]; Spot < End; Spot++)
		{
			if ((ScriptBuf[Spot] == CMD_POP_SVAR) && (ScriptBuf[Spot - 2] == CMD_PUSH_CON) && (ScriptBuf[Spot - 1] != 0) && (ScriptBuf[Spot - 4] == CMD_JUMP_IF) && (ScriptBuf[Spot - 6] == CMD_PUSH_SVAR))
			{
	...
	if (Ret >= 0)
	{
		...
		for (Spot = OffsetArray[Ret]; Spot < End; Spot++)
		{
			if ((ScriptBuf[Spot] == CMD_POP_SVAR) && (ScriptBuf[Spot - 2] == CMD_PUSH_FVAR) && (ScriptBuf[Spot - 1] == 0))
			{
	...
	if (Ret >= 0)
	{
		...
		for (Spot = OffsetArray[Ret]; Spot < End; Spot++)
		{
			if ((ScriptBuf[Spot] == CMD_POP_SVAR) && (ScriptBuf[Spot - 1] == OP_GET_UNITVAL) && (ScriptBuf[Spot - 3] == CMD_PUSH_CON) && (ScriptBuf[Spot - 2] == 27))
			{
	...
	if (Ret >= 0)
	{
		...
		for (Spot = OffsetArray[Ret]; Spot < End; Spot++)
		{
			if ((ScriptBuf[Spot] == CMD_POP_SVAR) && (ScriptBuf[Spot - 2] == CMD_PUSH_CON) && (ScriptBuf[Spot - 1] != 0) && (ScriptBuf[Spot + 2] == CMD_CALL_SCRPT) && (strnicmp(PrintScriptName(ScriptBuf[Spot + 3]), "startbuild", MAX_PATH) == 0))
			{
	...
	for (x = 0; x < NumScripts; x++)
		if ((strnicmp(PrintScriptName(x), "MoveWatcher", MAX_PATH) == 0) || (strnicmp(PrintScriptName(x), "MoveRate", MAX_PATH) == 0))
			Ret = x;
	if (Ret >= 0)
	{
		if (Ret + 1 == NumScripts)
		...
		for (Spot = OffsetArray[Ret]; Spot < End; Spot++)
		{
			if ((ScriptBuf[Spot] == CMD_SIG_MSK) && (ScriptBuf[Spot - 2] == CMD_PUSH_CON) && (ScriptBuf[Spot - 1] != 0))
			{
				Signal.Name = "SIG_MOVE";
				Signal.Val = ScriptBuf[Spot - 1];
				AddSignal(Signal);
	...
	if (Ret >= 0)
	{
		...
		for (Spot = OffsetArray[Ret]; Spot < End; Spot++)
		{
			if ((ScriptBuf[Spot] == CMD_SIG_MSK) && (ScriptBuf[Spot - 2] == CMD_PUSH_CON) && (ScriptBuf[Spot - 1] != 0) && ((ScriptBuf[Spot + 3] == CMD_JUMP_IF) || (ScriptBuf[Spot + 6] == CMD_JUMP_IF)))
			{
...
int CCobCodec::GetINIInfo()
{
	...
	if (!ReadFile(INI, buf, Size, &BR, NULL))
		goto EXIT;
	start = buf;
	...
	for (out = false;;)
	{
		...
		if (ComFunc.NumArgs)
			ComFunc.Args = new ARG[ComFunc.NumArgs];
		else
			ComFunc.Args = 0;
