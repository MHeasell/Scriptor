...
void CDlgWnd::OnOpen()
{
	...
	else
	{
		...
		if (attr == 0xFFFFFFFF)
		{
			...
			else if (len >= 2 && ss[0] == '\\' && ss[1] == '\\' && ((ss2 = strchr((const char*)ss + 2, '\\')) == NULL || strchr(ss2 + 1, '\\') == NULL))
			{
				AfxMessageBox(ss + _T("\nThis is not a valid folder."));
				pEdit->SetFocus();
				return;
...
void CDlgWnd::CheckDir(const CString& ss)
{
	...
	if (strSaved.IsEmpty() || strSaved[0] == '.' || strNew.CompareNoCase(strSaved) == 0 || strNew.CompareNoCase(strSaved + '\\') == 0)
	{
...
void CDirEdit::OnChar(UINT nChar, UINT nRepCnt, UINT nFlags)
{
	...
	else if (nChar == '\r' || nChar == '\n')
	{
		...
		if (strcmp(ss, "\\") != 0 && strcmp(ss, "\\\\") != 0 && strcmp((const char*)ss + 1, ":\\") != 0 && len > 0 && ss[len - 1] == '\\')
		{
			ss = ss.Left(--len);
		}

		if (len == 0 || len == 1 && ss[0] == '\\' || len >= 2 && ss[0] == '\\' && ss[1] == '\\' && strchr((const char*)ss + 2, '\\') == NULL || len == 2 && ss[1] == ':' || len == 3 && ss[1] == ':' && ss[2] == '\\')
		{
